<template>
  <div class="map-box">
    ...
    <m-hanlde-new @opClick="hdOpClick" @legendClick="hdLegendClick" class="handle"></m-hanlde-new>
    <m-popup @close="closePopup" ref="mPopup">
      <m-area-form ref="areaPopForm" v-if="handleType === 'area'" :areaData="curAreaData" @cancel="closePopup" @save="areaSave"></m-area-form>
      
      <m-device-form ref="devicePopForm" v-if="handleType === 'device'" :deviceData="curDeviceData" @cancel="closePopup" @save="deviceSave"></m-device-form>
...
export default {
  ...
  methods: {
    ...
    unBindPointerMove () {
      this.map.removeEventListener('pointermover')
    },
    ...
    // 根据 type(polygon, point)类型增加一个绘制矢量图的图层
    addDrawLayer (type) {
      this.drawSource = new VectorSource()
      this.drawLayer = new VectorLayer({
        source: this.drawSource,
        // style: this.getDrawStyleByType(type)
        style: this.getElementStyle({ type: type === 'Polygon' ? 'default' : 'location' })
      })
      this.map.addLayer(this.drawLayer)
      if (type === 'Polygon') {
        this.curDraw = new Draw({ source: this.drawSource, type: type, style: this.getDrawStyleByType(type) })
        ...
        // this.curDraw = new Draw({ source: this.drawSource, type: type })
        // this.map.addInteraction(this.curDraw)
        // this.map.removeEventListener('click')
        this.map.on('singleclick', (evt) => {
          // let coordinate = evt.coordinate
          this.bindCoordinate = evt.coordinate
          this.handleType = 'device'
          this.curDeviceData = {
            point: this.bindCoordinate
          }
          this.overlay.setPosition(this.bindCoordinate)
          this.createMarker(evt)
        })
      }
    },
    // 创建 marker
    createMarker (evt) {
      ...
      let markerSrc = this.drawLayer.getSource()
      const iconDefault = new Feature({
        geometry: new Point(this.bindCoordinate),
        name: ''
      })
      // iconDefault.setStyle(new Style({  // marker 默认样式 是否需要默认图标？
      //   /* image: new Icon(({
      //     anchor: [0.5, 46],
      //     anchorXUnits: 'fraction',
      //     anchorYUnits: 'pixels',
      ...
      markerSrc.addFeatures([iconDefault])
      this.curFeature = iconDefault
    },

    // 根据类型返回对应绘制的style
    getDrawStyleByType (type) {
      // 非 Polygon style重置
      return type === 'Polygon'
        ? new Style({
    ...
    // 关闭 popover 确保每次都是初始值
    resetPopForm () {
      if (this.handleType === 'device') {
        let deviceForm = this.$refs.devicePopForm.deviceForm
        for (let key in deviceForm) {
          deviceForm[key] = key === 'deviceType' ? 1 : ''
        }
      } else if (this.handleType === 'area') {
        let areaForm = this.$refs.areaPopForm.areaForm
        for (let key in areaForm) {
          areaForm[key] = key === 'areaType' ? 1 : ''
        }
      }
    },
    // 处理弹窗关闭事件
    ...
    deviceSave (data) {
      ...
      let featureData = { name: data.name, type: deviceType }
      this.curFeature.setProperties(featureData)
      this.curFeature.setStyle(this.getElementStyle(featureData))
      this.resetPopForm()
    }
  }
}
